...
<%
    ...
    try {
        ...
        // Get admin details
        String adminQuery = "SELECT first_name, last_name, image_url FROM admin_users WHERE id = ? LIMIT 1";
        pstmt = conn.prepareStatement(adminQuery);
        pstmt.setInt(1, 1); // Replace with actual admin ID from session
        rs = pstmt.executeQuery();
        
        if (rs.next()) {
            adminName = rs.getString("first_name") + " " + rs.getString("last_name");
            adminImage = rs.getString("image_url");
        }
        ...
        String ticketsQuery =
            ...
            "JOIN   users u ON st.user_id = u.user_id " +   // ← fixed here
                ...
                <div class="p-6">
                    <form action="Contact-Support.jsp" method="post">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="response-<%= ticket.get("id") %>" name="admin_response" rows="4" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500"><%= ticket.get("admin_response") != null ? ticket.get("admin_response") : "" %></textarea>
                        </div>
