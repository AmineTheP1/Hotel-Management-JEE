    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            applyFiltersBtn.addEventListener('click', function() {
                // Get selected hotel and date range values
                const selectedHotel = document.getElementById('hotel-filter').value;
                const selectedDateRange = document.getElementById('date-range').value;
                
                // Show loading state
                applyFiltersBtn.innerHTML = '<i class="fas fa-spinner fa-spin mr-1"></i> Applying...';
                ...
                setTimeout(function() {
                    ...
                    // Reset button state
                    applyFiltersBtn.innerHTML = '<i class="fas fa-filter mr-1"></i> Apply Filters';
            ...
            // Function to apply filters to the dashboard data
            function applyFilters(filterData) {
                // This function would typically update the dashboard with filtered data
                // For now, we'll just show what filters were applied
                
                // Update page title with filter info
                const filterInfo = document.createElement('span');
                filterInfo.className = 'ml-2 text-sm font-normal text-gray-500';
                filterInfo.textContent = `(Filtered: ${filterData.dateRange}, Hotel: ${filterData.hotel})`;
                ...
                const existingFilterInfo = dashboardTitle.querySelector('span');
                if (existingFilterInfo) {
                    dashboardTitle.removeChild(existingFilterInfo);
                }
                
                // Add the new filter info
                dashboardTitle.appendChild(filterInfo);
                
                // In a real implementation, you would:
                // 1. Make an AJAX request to get filtered data from the server
                // 2. Update the charts and statistics with the new data
                // 3. Refresh the displayed information
            }
            ...
            function showNotification(message) {
                ...
                notification.className = 'fixed top-4 right-4 bg-green-500 text-white px-4 py-2 rounded-lg shadow-lg z-50 transform transition-transform duration-300 translate-y-0';
