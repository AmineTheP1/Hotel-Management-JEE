...
<%@ page import="java.util.Calendar,java.util.List,java.util.Map,java.util.ArrayList,java.util.HashMap,java.util.Date,java.util.Set,java.util.HashSet" %>
    ...
    <div class="flex">
        ...
        <main class="flex-1 p-6">
            <div class="flex justify-between items-center mb-8">
                ...
                <a href="create-booking.jsp" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition duration-200 flex items-center">
                        ...
                        <div id="custom-date-range" class="w-full hidden">
                            <div class="flex gap-2">
                                <div class="w-1/2">
                                    ...
                                    <input type="date" id="date-from" class="w-full px-3 py-2 border rounded-lg focus:ring-blue-500 focus:border-blue-500">
                                </div>
                                <div class="w-1/2">
                                    <label for="date-to" class="block text-sm font-medium text-gray-700 mb-1">To</label>
                                    <input type="date" id="date-to" class="w-full px-3 py-2 border rounded-lg focus:ring-blue-500 focus:border-blue-500">
                                </div>
                            </div>
                        </div>
                        
                        <div class="w-full md:w-auto flex items-end">
                            <button id="apply-filters" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition duration-200">
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    // Show/hide custom date range when "Custom Range" is selected
                    ...
                    document.getElementById('apply-filters').addEventListener('click', function() {
                        ...
                        bookingRows.forEach(row => {
                            const hotelValue = row.getAttribute('data-hotel');
                            const statusValue = row.getAttribute('data-status');
                            const checkInDate = new Date(row.getAttribute('data-check-in'));
                            const checkOutDate = new Date(row.getAttribute('data-check-out'));
                            
                            let showRow = true;
                            
                            // Apply hotel filter
                            if (hotelFilter !== 'all' && hotelValue !== hotelFilter) {
                                showRow = false;
                            }
                            ...
                            if (dateFilter !== 'all') {
                                ...
                                if (dateFilter === 'today' && (checkInDate < today || checkInDate >= tomorrow)) {
                                    showRow = false;
                                } else if (dateFilter === 'tomorrow' && (checkInDate < tomorrow || checkInDate >= new Date(tomorrow.getTime() + 86400000))) {
                                    showRow = false;
                                ...
                                } else if (dateFilter === 'next-week' && (checkInDate < nextWeekStart || checkInDate >= nextWeekEnd)) {
                                    showRow = false;
                                } else if (dateFilter === 'this-month' && (checkInDate < thisMonthStart || checkInDate > thisMonthEnd)) {
                                    showRow = false;
                                    ...
                                    if (dateFrom && dateTo) {
                                        if (checkInDate < dateFrom || checkInDate > dateTo) {
                                            showRow = false;
                                        }
                                    }
                                }
